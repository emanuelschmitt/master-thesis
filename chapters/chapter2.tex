...
In order to show the feasibility for a full software keyboard, \citeauthor{Accessory} showed a second attempt to the problem by creating \textit{ACCessory}. \textit{ACCessory} is an Android application with functionalities similar to the previously mentioned \textit{Touchlogger}. However, the application significantly differ in it's tap area granularity providing two separate modes for tap inputs. The first consists of tap areas arranged in a 60-cell grid whereas the second is a QWERTY keyboard in landscape orientation. Having extracted features mainly from the time-domain, a classification using the Random Forests algorithm reached an accuracy of 24.5\% for the 60-cell grid with a dataset containing 1300 keystrokes. As the grid-cell experiment focused on recognizing individual keystrokes, the software keyboard experiment in the paper focused on cracking passwords by combining the keystrokes into a sequence. Assuming recognition errors in individual characters, the researchers created a ranked list of candidate passwords by running a maximum likelihood search for the most probable classification errors for an obtained password. Here, 6 out of 99 password could be inferred under 4.5 median trials\footnote{One trails refers to traversing down one item of the candidate list produced by the maximum likelihood search.}, whereas 59 out of 99 could be inferred in $2^{15}$ median trials. As a result, due to low accuracy of the learning system, the researchers could significantly reduce the search space for a password or text, however, could not immediately extract information.\\

The most comprehensive followup study to date concerning the subject was conducted by \citeauthor{Tapprints} and differs from \texit{Accessory} and \textit{Touchlogger} in many important ways. While both studies are both evaluated on the Android smartphones, \textit{TapPrints} investigates tap inference on both iOS and Android platforms including smartphone and tablet devices. Furthermore, \textit{TapPrints} combines the accelerometer and gyroscope for a more in depth feature extraction taking correlations between both motions sensors into account. In addition, the training dataset contains over 40.000 taps 
