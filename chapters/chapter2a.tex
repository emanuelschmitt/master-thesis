...
\begin{figure}[h]
  ...
  \def\layersep{2cm}
    ...
    \tikzstyle{neuron}=[circle,draw=black!80, thick,minimum size=17pt,inner sep=0pt]
...
\begin{equation}
  \delta_j =
  \begin{cases}
    \varphi'(net_j)(o_j - \^{y}_j) & \text{node $j \in L_k$}\\
...
% In order for a feedforward network to be trained onto a supervised learning, task a network is presented with training examples which each consist of a set of feautres  with a corresponding label $y$. The aim of the of the backpropagation algorithm is to minimize the global MSE error 



% by adjusting the weights and biases of individual nodes. For this purpose, gradient descent is used to minimize minimize this error. To determine which of the unit's are causing errors each partial derivative of E with respect to the individual weights is computed.\\



% Now we can calculate the partial derivative of E in respect to an individual weight of a neural unit


% We can compute the change of the weight $\Delta w_{ij}$ between nodes $i$ and $j$. 

% $\^{y}_j$ is the desired output of node $j$. $o_i$ being the output of the neuron.
% \begin{equation}
%   \Delta w_{ij} = -\eta \frac{\delta E}{\delta w_{ij}} = - \eta \delta_j o_i
% \end{equation}
...
%   \delta_j =
%   \begin{cases}
%     \varphi'(net_j)(o_j - \^{y}_j) & \text{if node j is output layer}\\
%     \varphi'(net_j) \sum_k \delta_k w_{jk} & \text{if node j is in output node}
%   \end{cases}
% \end{equation}


% Through forwardpropagation the output $\^{y}$ is computed based on the equation (3.17) with the global error value based on the following equation:

% \begin{equation}
%   E = \frac{1}{2}\sum_{m} (\^{y} - y)^2
% \end{equation}

% In this equation, the global error between the output and the desired output of the network is measured. In order to reduce the error between the ouput and the disired output, the weights need to be updated according to error that they produce.
...
% desired output is measured and the weights are updated to minimize this error at each
% node in each layer. 

% the objective to minimize the error between the computed output label $\^{y}$ of the network and target $y$. The error function for a given weight can be expressed as follows:
% % \begin{equation}
% %   E = \frac{1}{2}\sum_j (y_j - \^{y}_j)
% % \end{equation}
% \begin{equation}
%   E(w_1, w_2, \dots, w_n, \alpha) = \frac{1}{2} (\^{y} - y)^2
% \end{equation}

% $w_k$ is the $m$-dimensional vector of weights between the inputs and the $k$th hidden node while $\alpha$ is the vector of weights between the hidden nodes and the output, $\^{y}$ denotes the output value of the network.\\

% The gradient descent method involves calculating the derivative of the squared error function with respect to the weights of the network. This is normally done using backpropagation.

% In an iterative process, we must compute the gradient of the error
% function E with respect to the weights by calculating the partial derivative with respect to each weight. After computation, the weights can be updated accordingly:

% \begin{equation*}
%   \alpha_k \leftarrow \alpha_k - \eta \frac{\delta E}{\delta \alpha_k} a_k
% \end{equation*}

% \begin{equation*}
%   w_{ik} \leftarrow \alpha_k - \eta x_i
% \end{equation*}

% where $\eta$ denotes the \textit{learning rate}
